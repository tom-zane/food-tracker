...
import { activityLevels } from '../../../utils/tdeeCalculator';
import { COLORS, SPACING } from '../../../styles/theme';

export const ProfileSettings = () => {
  const { profile, updateProfile } = useContext(SettingsContext);

  // Helper to handle Android text inputs safely
  const handleNumericInput = (key, value) => {
    // Strips everything except numbers and decimals
    const cleanValue = value.replace(/[^0-9.]/g, ''); 
    // Allow empty string so the user can delete numbers without it snapping to "0"
    updateProfile({ [key]: cleanValue === '' ? '' : cleanValue });
  };

  console.log("profile from ProfileSettings.jsx", profile);
      ...
      <View style={styles.rowInputs}>
        <View style={[styles.inputGroup, { flex: 1 }]}>
          ...
          <TextInput 
            ...
            onChangeText={(v) => handleNumericInput('weight', v)} 
          />
        </View>
        <View style={[styles.inputGroup, { flex: 1 }]}>
          ...
          <TextInput 
            ...
            onChangeText={(v) => handleNumericInput('height', v)} 
          />
        </View>
        <View style={[styles.inputGroup, { flex: 0.8 }]}>
          ...
          <TextInput 
            ...
            onChangeText={(v) => handleNumericInput('age', v)} 
          />
        </View>
      </View>
      ...
      <View style={styles.pickerContainerHorizontal}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {activityLevels.map(level => (
            <TouchableOpacity 
              key={level.value}
              style={[styles.chip, profile.activityLevel === level.value && styles.chipSelected]}
              onPress={() => updateProfile({ activityLevel: level.value })}
            >
              <Text style={[styles.chipText, profile.activityLevel === level.value && styles.chipTextSelected]}>
                {level.label}
              </Text>
            </TouchableOpacity>
          ))}
